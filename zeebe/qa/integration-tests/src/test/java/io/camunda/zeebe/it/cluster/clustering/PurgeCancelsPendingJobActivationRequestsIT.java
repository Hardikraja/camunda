...
/**
 * Verifies that pending long-poll job activation requests are cancelled when a cluster purge
 * occurs. The purge increments the cluster incarnation number, which triggers the gateway to fail
 * all open long-poll requests with a "purge" error so that clients can reconnect cleanly.
 */
@ZeebeIntegration
final class PurgeCancelsPendingJobActivationRequestsIT {
  ...
  @Test
  void shouldCancelPendingGrpcLongPollRequestsOnPurge() {
    try (final var client = cluster.newClientBuilder().preferRestOverGrpc(false).build()) {
      assertPendingActivationCancelledByPurge(client, "grpc-purge-test");
    }
  }
  ...
  void shouldCancelPendingRestLongPollRequestsOnPurge() {
    try (final var client = cluster.newClientBuilder().preferRestOverGrpc(true).build()) {
      assertPendingActivationCancelledByPurge(client, "rest-purge-test");
    }
  }

  private void assertPendingActivationCancelledByPurge(
    ...
    assertThat((java.util.concurrent.CompletionStage<ActivateJobsResponse>) pendingActivation)
