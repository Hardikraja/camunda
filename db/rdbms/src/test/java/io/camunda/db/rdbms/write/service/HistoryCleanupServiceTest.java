...
class HistoryCleanupServiceTest {
  ...
  void testCalculateNewDurationUsageLimitEnforcedWhenCleanupTookTooLong() {
    ...
    // then - usage limit (25%) requires interval >= 48s, larger than the halved interval 20s
    assertThat(nextDuration).isEqualTo(Duration.ofSeconds(48));
  }
  ...
  static Stream<Arguments> ensureWithinMaxHistoryCleanupUsageTestCases() {
    return Stream.of(
        Arguments.of(
            ...
            0.25,
            Duration.ofSeconds(48)), // 12s / 0.25 = 48s
        Arguments.of(
            "does not reduce interval when cleanup was fast",
            Duration.ofSeconds(20),
            Duration.ofSeconds(1),
            0.25,
            Duration.ofSeconds(20)), // 1s / 0.25 = 4s < 20s, proposed wins
        Arguments.of(
            "applies stricter usage limit",
            Duration.ofSeconds(20),
            Duration.ofSeconds(12),
            0.10,
            Duration.ofSeconds(120)), // 12s / 0.10 = 120s
        Arguments.of(
            "allows higher usage before enforcing",
            Duration.ofSeconds(20),
            Duration.ofSeconds(12),
            0.50,
            Duration.ofSeconds(24)), // 12s / 0.50 = 24s
        Arguments.of(
